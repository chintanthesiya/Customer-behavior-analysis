...
--there are some commenly question and thair problem solving using sql query

--1.what is the total revenue generated by male vs female customers?

select gender,sum(purchase_amount)as revenue from customer group by gender

--2.which customers used a discount but still spend more than average purchase amount?
...
select shipping_type,round(
avg(purchase_amount),2) from customer
...
--6.which 10 products have the highest percentages of purchases with discount applied?
...
--7.segment customers into new,returning and loyal based on thair total number of previous purchases,and show the count of each segment
...
--8.what are the top 3 purchased products whithin each category?
